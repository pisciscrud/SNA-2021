...
#pragma endregion
	...
	bool Mfst::start(std::ostream& stream_out) {
		...
		case LENTA_END:
			MFST_TRACE4("------>LENTA_END")
				stream_out << "------------------------------------------------------------------------------------------   ------" << std::endl;
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: number of strings %d, the syntactic analysis was performed without errors", 0, lex.table[lex.size - 1].line);
			stream_out << std::setw(4) << std::left << 0 << "number of strings " << lex.table[lex.size - 1].line << ", the syntactic analysis was performed without errors" << std::endl;
			rc = true;
			break;
		case NS_NORULE:
	...
	char* Mfst::getDiagnosis(short n, char* buf) {
		...
		if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0) {
			...
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: string %d,	%s", err.id, lex.table[lpos].line, err.message);
			rc = buf;
		}
